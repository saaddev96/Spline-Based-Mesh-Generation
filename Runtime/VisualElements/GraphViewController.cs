...
namespace THLT.SplineMeshGeneration.Scripts.VisualElements
{
    ...
    public class GraphViewController
    {
        ...
        public void SelectVertex(Vector2 snapPoint)
        {
            ...
            var verticesAtPos = CurrentShape.VertsData.Where(x => x.Point == snapPoint).OrderBy(x => x.Index).ToArray();
        ...
        public void UpdateSelectedPointPosition(Vector2 snapPoint)
        {
            ...
            if (CurrentShape.VertsData.Exists(x => x.Point == snapPoint) &&
        ...
        public void UpdateSelectedPointNormal(int index)
        {
            if (CurrentShape == null) return;
            if (SelectedVertex.Index == -1) return;
            if (!CurrentShape.NormalLinesDictionary.TryGetValue(index, out var targetLine))
                return;
            if (targetLine != null)
            {
                targetLine.Start = CurrentShape.VertsData[index].Point;
                targetLine.End = CurrentShape.VertsData[index].Point + CurrentShape.VertsData[index].Normal;
            }
