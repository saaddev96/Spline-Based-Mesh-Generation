...
namespace THLT.SplineMeshGeneration.Scripts.Editor
{
    public class Mesh2DDrawerEditor :EditorWindow
    {
        ...
        void BindElements()
        {
            ...
            HandleOnValueChanged<Vector2Field, Vector2>(_selectedVertexPositionField, OnSelectedVertexPointChanged);
        ...
        void OnSelectedVertexPointChanged(ChangeEvent<Vector2> evt)
        {
            var selectedVertIndex = _graphViewController.SelectedVertex.Index;
            _graphViewController.CurrentShape.VertsData[selectedVertIndex] = _graphViewController.SelectedVertex.SetPosition(evt.newValue);
            _graphViewController.UpdateSelectedPointPosition(evt.newValue);
        }
